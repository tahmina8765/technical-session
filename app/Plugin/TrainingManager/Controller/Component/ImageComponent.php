...
class ImageComponent extends Component {
    ...
    function upload_image_and_thumbnail($data, $maxw, $maxh, $thumbscalew, $thumbscaleh, $folderName) {
        if (strlen($data['name']) > 4) {
            ...
            if (!is_dir($homeuploaddir))
                mkdir($homeuploaddir, 0755, true);
            if (!is_dir($biguploaddir))
                mkdir($biguploaddir, 0755, true);
            if (!is_dir($smalluploaddir))
                mkdir($smalluploaddir, 0755, true);
            $filetype       = $this->getFileExtension($data['name']);
            $filetype       = strtolower($filetype);
            if (($filetype != "jpeg") && ($filetype != "jpg") && ($filetype != "gif") && ($filetype != "png")) {
                //verify the extension
                //modified by zadid
                /* if($data['type'] == 'application/pdf' || $data['type'] == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ){
                  move_uploaded_file($data['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/app/webroot/img/');
                  die($data['name']);
                  return $data['name'];
                  }else{ */
                return;
            ...
            if (is_uploaded_file($data['tmp_name'])) {
                ...
                if (!copy($data['tmp_name'], "$tempfile")) {
                     ...
                     */
                    $this->resizeImage('resizeCrop', $tempuploaddir, $filename, $homeuploaddir, $filename, 400, 250, 85);
                    /*
        ...
        if ($newWidth OR $newHeight) {
            ...
            if (file_exists($dstimg)) {
                ...
                switch ($ext) {
                    ...
                    case 'png' :
                        $pngquality = 8;
                        imagepng($newImage, $dstimg, $pngquality);
                        break;
                    case 'jpg' :
                    case 'jpeg' :
